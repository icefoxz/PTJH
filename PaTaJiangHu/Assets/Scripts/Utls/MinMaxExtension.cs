...
namespace Utls
{
    ...
    public static class MinMaxExtension
    {
        ...
        /// <param name="includedMax"></param>
        /// <returns></returns>
        public static bool InMinMaxRange(this MinMaxInt clause, int value, bool includedMax = true)
        {
            //上限如果小于0(-1)直接判定是否状态为0
            if (clause.Max < 0) return value == 0;
            var skipMin = clause.Min <= 0;
        ...
        /// <param name="includedMax"></param>
        /// <returns></returns>
        public static bool InMinMaxRange(this RangedInt clause, int value, bool includedMax = true)
        {
            //上限如果小于0(-1)直接判定是否状态为0
            if (clause.Max < 0) return value == 0;
            var skipMin = clause.Min <= 0;
