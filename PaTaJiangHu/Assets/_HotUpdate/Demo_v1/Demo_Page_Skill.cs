    ...
    internal class Demo_Page_Skill : PageUiManagerBase
    {
        ...
        protected override void Build(IView v)
        {
            ...
            view_skillDev = new View_skillDev(v.GetObject<View>("view_skillDev"),
                (s, i) => { SkillSelected(s, i); },
                (guid, skill, index) => { Agent.Skill_BookComprehend(guid, skill, index); },
                Evolve,
                SkillUses,
                SkillForget,
                () => { Agent.Skill_Comprehend(_selectedDizi.Guid, _selectedSkill, _selectedIndex); },
                true);
            view_diziList = new View_DiziList(v.GetObject<View>("view_diziList"),
                guid => { Agent.SkillPage_Show(guid); });
        }
